...
for batch_idx, (data, y) in enumerate(test_loader):
    ...
    data = torch.cat([data[0],data[3]],dim=1)
    ...
    # 前向传播，获取指定层的输出
    output1, output2, output3 = target_layer_1(data), target_layer_2(data), target_layer_3(data)
    # 将特征图转为numpy数组
    ...
    feature_maps1 += np.sum(feature_map2, axis=0)
    print(feature_maps1)
    print(i)
